...
#  FileName:        627.cc
#  Description:     UVa Online Judge - 627
=============================================================================*/
#include <bits/stdc++.h>
...
void bfs(int st, int en) {
    vi visited(MAXN), dist(MAXN, 0), par(MAXN);
    queue<ii> Q;
    Q.push({st, -1});
...
int main() {
    ...
    while (getline(cin, line)) {
        ...
        for (int i = 1; i <= N; i++) {
            ...
            if (tokens.size() == 1 or tokens[1] == "") continue;
            vs targets = split(tokens[1], regex(","));
            for (auto& target : targets) V[source].push_back(stoi(target));
            sort(V[source].begin(), V[source].end());
        }
        ...
        while (NQ--) {
            ...
            bfs(stoi(tokens[0]), stoi(tokens[1]));
        }
    }
