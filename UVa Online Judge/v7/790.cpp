...
#  FileName:        790.cpp
#  Description:     UVa Online Judge - 790
=============================================================================*/
#include <algorithm>
#include <cstdio>
#include <cstring>
...
struct contestant {
    contestant() {}
    contestant(int score, int consumed_time, int id)
        : score(score), consumed_time(consumed_time), id(id) {}
    int score, consumed_time, id;

    void init() { score = consumed_time = id = 0; }

    bool operator<(struct contestant x) const {
        if (isParticipate[id] != isParticipate[x.id])
            return isParticipate[id] > isParticipate[x.id];
        if (score != x.score) return score > x.score;
        if (consumed_time != x.consumed_time)
            return consumed_time < x.consumed_time;
        return id < x.id;
    }
} A[30];
char str[400];
...
int main() {
    ...
    while (TT--) {
        ...
        for (int i = 1; i <= 25; i++)
            A[i].id = i, A[i].consumed_time = A[i].score = 0;
        ...
        for (int i = 1; i <= 29; i++) {
            ...
            if (i > 1 and (A[i].score != A[i - 1].score or
                           A[i].consumed_time != A[i - 1].consumed_time))
                rank = i;
            if (A[i].score)
                printf("%4d %4d %4d %10d\n", rank, A[i].id, A[i].score,
                       A[i].consumed_time);
            else
                printf("%4d %4d\n", rank, A[i].id);
        }
