...
#  FileName:        11800.cc
#  Description:     UVa Online Judge - 11800
=============================================================================*/
#include <bits/stdc++.h>
...
double canonical(double theta) {
    if (theta > M_PI) return -(2.0 * M_PI - theta);
    return theta;
}
...
void solve() {
    ...
    sort(P + 1, P + 5, [](point2d& x, point2d& y) {
        if (fabs(x.y - y.y) > EPS) return x.y < y.y;
        return x.x < y.x;
    });

    sort(P + 2, P + 5, [](point2d& x, point2d& y) {
        return canonical(angle(x - P[1], point2d(1, 0))) <
               canonical(angle(y - P[1], point2d(1, 0)));
    });

    double d12 = abs(P[2] - P[1]), d13 = abs(P[3] - P[1]),
           d14 = abs(P[4] - P[1]), d23 = abs(P[3] - P[2]),
           d24 = abs(P[4] - P[2]), d34 = abs(P[4] - P[3]);

    double angle1 =
        acos((d24 * d24 - d12 * d12 - d14 * d14) / (2.0 * d12 * d14));
    double angle2 =
        acos((d13 * d13 - d12 * d12 - d23 * d23) / (2.0 * d12 * d23));
    double angle3 =
        acos((d24 * d24 - d34 * d34 - d23 * d23) / (2.0 * d34 * d23));
    double angle4 =
        acos((d13 * d13 - d14 * d14 - d34 * d34) / (2.0 * d14 * d34));
    ;

    if (fabs(d12 - d23) < EPS and fabs(d23 - d34) < EPS and
        fabs(d34 - d14) < EPS and fabs(angle1 - M_PI / 2.0) < EPS and
        fabs(angle2 - M_PI / 2.0) < EPS and fabs(angle3 - M_PI / 2.0) < EPS and
        fabs(angle4 - M_PI / 2.0) < EPS) {
        cout << "Square" << endl;
        return;
    }

    if (fabs(d12 - d34) < EPS and fabs(d23 - d14) < EPS and
        fabs(angle1 - M_PI / 2.0) < EPS and fabs(angle2 - M_PI / 2.0) < EPS and
        fabs(angle3 - M_PI / 2.0) < EPS and fabs(angle4 - M_PI / 2.0) < EPS) {
        cout << "Rectangle" << endl;
        return;
    }
    ...
    if (fabs(d12 - d34) < EPS and fabs(d23 - d14) < EPS) {
        cout << "Parallelogram" << endl;
        return;
    }

    if (fabs(angle1 + angle2 - M_PI) < EPS or
        fabs(angle1 + angle4 - M_PI) < EPS) {
        cout << "Trapezium" << endl;
        return;
    }
