...
#  FileName:        11566.cc
#  Description:     UVa Online Judge - 11566
=============================================================================*/
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    while (cin >> N >> x >> T >> K) {
        ...
        vector<vector<int> > dp_favors(MAX_ITEMS,
                                       vector<int>(MAX_TOTALPRICE, -1));
        dp_favors[0][0] = 0;
            ...
            for (int tt = 1; tt <= 2; tt++) {
                for (int n_item = 2 * i; n_item >= 0; n_item--) {
                    ...
                    for (int j = MAX_TOTALPRICE - 1; j >= 0; j--) {
                        ...
                        dp_favors[n_item + 1][j + price] =
                            max(dp_favors[n_item + 1][j + price],
                                dp_favors[n_item][j] + total_favors);
                        max_favors =
                            max(max_favors, dp_favors[n_item + 1][j + price]);
                    }
                }
            }
        }
        double rounded_max_favors =
            round((double)max_favors / (N + 1) * 100.0) / 100.0;
