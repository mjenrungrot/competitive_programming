...
int main(){
    ...
    while(cin >> command){
        ...
        if(command == "RANGE"){
            ...
            for(int i=0,curr=params[0];i<params[2];i++,curr+=params[1]){
            ...
            while(not Q.empty()){
                ...
                int dist;
                if(front.second <= 0) dist = INF;
                else dist = query(fw_counter) - query(front.second); // if dist >= cache_size, it will miss.

                for(int i=1;i<=N;i++){
                    if(dist > caches[i]) misses[i]++;
                    else break;
                }
                if(front.second > 0) add(front.second, -1);
                fw_counter++;
            }
            for(int i=1;i<=N;i++) cout << (i>1 ? " ":"") << misses[i];
            cout << endl;
        }
    }
    return 0;
