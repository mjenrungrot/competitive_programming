...
#  FileName:        12796.cc
#  Description:     UVa Online Judge - 12796
=============================================================================*/
#include <bits/stdc++.h>
...
struct matrix fast_pow(struct matrix& x, int n) {
    if (n == 0) return I(static_cast<int>(x.A.size()));
...
void solve() {
    ...
    for (int i = 0; i < N; i++)
        for (int j = 0; j < K; j++) {
            assert(buttons[i][j] - 1 < N);
            A.A[i][buttons[i][j] - 1]++;
        }

    struct matrix B = fast_pow(A, L);
    cout << B.A[S - 1][T - 1] << endl;
}
...
int main() {
    ...
    while (cin >> N >> L) {
        cin >> S >> T;
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < K; j++) cin >> buttons[i][j];
        }
        solve();
    }
