...
#  FileName:        129.cc
#  Description:     UVa Online Judge - 129
=============================================================================*/
#include <algorithm>
#include <iostream>
#include <string>
...
int main() {
    ...
    while (cin >> N >> L) {
        ...
        int n_lines =
            (ans.length() + N_GROUPS_PER_LINE * N_CHARS_PER_GROUP - 1) /
            (N_GROUPS_PER_LINE * N_CHARS_PER_GROUP);
        for (int i = 0; i < n_lines; i++) {
            if (i) cout << endl;
            int start = N_GROUPS_PER_LINE * N_CHARS_PER_GROUP * i;
            int end = min(start + N_GROUPS_PER_LINE * N_CHARS_PER_GROUP,
                          (int)ans.length());
            int n_group_this_line =
                (end - start + N_CHARS_PER_GROUP - 1) / N_CHARS_PER_GROUP;
            for (int group = 0; group < n_group_this_line; group++) {
                if (group) cout << " ";
                int start_group = start + N_CHARS_PER_GROUP * group;
                int end_group =
                    min(start_group + N_CHARS_PER_GROUP, (int)ans.length());
