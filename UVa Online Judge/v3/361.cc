...
#  FileName:        361.cc
#  Description:     UVa Online Judge - 361
=============================================================================*/
#include <bits/stdc++.h>
...
bool check(bool flag, polygon::polygon2d &P, polygon::point2d &query, const double EPS=1e-9){
    if(P.points.size() <= 3){
        // have < 3 people
        ...
        // check point
        for(auto &p: P.points){
            if(abs(p - query) < EPS) return true;
        }

        // check line
        for(int i=0;i<P.points.size();++i) for(int j=i+1;j<P.points.size();++j){
            double d1 = abs(P.points[i] - query);
            double d2 = abs(P.points[j] - query);
            double d3 = abs(P.points[i] - P.points[j]);
            if(fabs(d1+d2-d3) < EPS) return true;
        }
    }
    return P.inside_polygon(query) >= 0;
}

void solve() {
    polygon::polygon2d cop, robber;

    for (int i = 1; i <= C; i++) {
        double x, y;
        cin >> x >> y;
        cop.points.emplace_back(x, y);
    }

    for (int i = 1; i <= R; i++) {
        double x, y;
        cin >> x >> y;
        robber.points.emplace_back(x, y);
    }

    polygon::polygon2d cop_hull = cop.convex_hull();
    polygon::polygon2d robber_hull = robber.convex_hull();

    for (int i = 1; i <= O; i++) {
        int x, y;
        cin >> x >> y;
        polygon::point2d query(static_cast<double>(x), static_cast<double>(y));

        bool safe = check(C<3, cop_hull, query);
        bool robbed = check(R<3, robber_hull, query);
