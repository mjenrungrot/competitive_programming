...
#  FileName:        357.cc
#  Description:     UVa Online Judge - 357
=============================================================================*/
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    vector<vector<long long>> memo(MAXLIMIT,
                                   vector<long long>(coins.size() + 1));
    for (int i = 0; i < coins.size(); i++) memo[0][i] = 1;

    for (int val = 1; val < MAXLIMIT; val++) {
        for (int coin_idx = 0; coin_idx < coins.size(); coin_idx++) {
            if (val - coins[coin_idx] >= 0 and
                memo[val - coins[coin_idx]][coin_idx]) {
