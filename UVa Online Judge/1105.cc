...
int main(){
    ...
    while(cin >> dx >> dy >> n >> q){
        ...
        for(int i=1;i<=n;i++){
            int x, y;
            cin >> x >> y;
            int new_y = y - x + dx; // y in [1,dy]; x-dx in [1-dx, 0]; y - x + dx in [1,dx+dy-1]
            int new_x = y + x; // x+y [2, dx+dy]
            A[new_y][new_x]++;
        }

        for(int i=1;i<size_rotated;i++) for(int j=1;j<size_rotated;j++){
        ...
        for(int i=1;i<=q;i++){
            ...
            for(int r=1;r<=dy;r++) for(int c=1;c<=dx;c++){
                int new_r = r - c + dx;
                int new_c = r + c;
                ...
                int count = A[lower_row][right_col] - A[lower_row][left_col-1] - A[upper_row-1][right_col] + A[upper_row-1][left_col-1];
