...
#  FileName:        12321.cc
#  Description:     UVa Online Judge - 12321
=============================================================================*/
#include <algorithm>
#include <cmath>
#include <iostream>
...
int main() {
    ...
    while (cin >> L >> G) {
        ...
        while (curr_idx < intervals.size()) {
            if (intervals[curr_idx].second <=
                curr_right) {  // fit in the last chosen interval
                curr_idx++;
                continue;
            }

            if (intervals[curr_idx].first <=
                curr_right) {  // can add this interval without making any space
                int best_val = intervals[curr_idx].second;
                int best_idx = curr_idx;
                int next_idx = curr_idx + 1;
                while (next_idx < intervals.size() and
                       intervals[next_idx].first <= curr_right) {
