...
#  FileName:        10145.cc
#  Description:     UVa Online Judge - 10145
=============================================================================*/
#include <bits/stdc++.h>
using namespace std;
        ...
        while (true) {
            ...
            if (mode == 'S') {
                map<int, set<int> >::iterator it =
                    itemToExclusiveTransactions.find(item_id);
                if (it != itemToExclusiveTransactions
                              .end()) {  // found exclusive lock on this item id
                    int n_diff_transactions =
                        itemToTransactions.count(item_id)
                            ? (itemToTransactions[item_id].count(t_id)
                                   ? itemToTransactions[item_id].size() - 1
                                   : itemToTransactions[item_id].size())
                            : 0;
                    if (n_diff_transactions >
                        0) {  // found exclusive lock with different transaction
