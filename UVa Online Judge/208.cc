...
bool reachable_test(){
    ...
    for(int i=0;i<MAX_NODE;i++) for(int j=0;j<MAX_NODE;j++) dp[i][j] = INF;
    for(int i=0;i<MAX_NODE;i++){
        dp[i][i] = 0;
        for(auto v: V[i]) dp[i][v] = 1;
    }
    for(int k=0;k<MAX_NODE;k++) for(int i=0;i<MAX_NODE;i++) for(int j=0;j<MAX_NODE;j++){
...
int main(){
    ...
    while(cin >> target){
        ...
        cout << "There are " << n_routes << " routes from the firestation to streetcorner " << target << "." << endl;
    }
    return 0;
}