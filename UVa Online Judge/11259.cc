...
#  FileName:        11259.cc
#  Description:     UVa Online Judge - 11259
=============================================================================*/
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    while (T--) {
        ...
        for (int i = 0; i < N_COINS; i++) {
            cin >> cs[i];
            for (int j = cs[i]; j < MAXLIMIT; j++) dp[j] += dp[j - cs[i]];
        }

        int Q;
        cin >> Q;
        for (int i = 0; i < Q; i++) {
            for (int j = 0; j < N_COINS; j++) cin >> ds[j];
            ...
            for (int mask = 0; mask < (1 << N_COINS); mask++) {
                ...
                for (int bit = 0; bit < N_COINS; bit++) {
                    if ((1 << bit) & mask) {
                        curr_val -=
                            (long long)(ds[bit] + 1) *
                            cs[bit];  // assume using at least ds[bit]+1 coins
                        parity = 1 - parity;
                    }
                }
                if (parity)
