...
#  FileName:        1111.cc
#  Description:     UVa Online Judge - 1111
=============================================================================*/
#include <bits/stdc++.h>
...
void solve() {
    ...
    polygon::polygon2d hull = poly.convex_hull(1e-6);
    double ans = 1e9;

    for (int i = 0; i < hull.points.size() - 1; ++i) {
        auto P0 = hull.points[i];
        double tmp = 0.0;
        auto P1 = hull.points[i+1];
        for (int j = 1; j < hull.points.size()-1; ++j) {
            auto P2 = hull.points[(i+1+j)%hull.points.size()];
            polygon::point2d _tmp;
            double d = polygon::dist_to_line(P2, P0, P1, _tmp);
            tmp = max(tmp, d);
        }
        ans = min(ans, tmp);
    }
