...
int main(){
	...
	while(scanf("%d %d",&N,&M) == 2){
		...
		for(int i=1;i<=M;i++) for(int j=1;j<=3;j++) scanf("%d",&A[i][j]);
		...
		do{
			...
			for(int i=1;i<=M;i++){
				while(!PQ[permu[nowid]].empty() and PQ[permu[nowid]].top().first == nowval){
					nowval = PQ[permu[nowid]].top().second + 1;
					tmp -= PQ[permu[nowid]].top().second - PQ[permu[nowid]].top().first + 1;
