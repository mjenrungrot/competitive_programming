...
void f(int idx, int curr_state){
    if(reachable) return;
    if(idx >= 3){
        if(conv(identifier, get(curr_state, idx-3), get(curr_state, idx-2), get(curr_state, idx-1)) != get(state, idx-2)){
            return;
        }
    }
    if(idx == N){
        // check head
        if(conv(identifier, get(curr_state, N-1), get(curr_state, 0), get(curr_state, 1)) != get(state, 0)) return;

        // check tail
        if(conv(identifier, get(curr_state, N-2), get(curr_state, N-1), get(curr_state, 0)) != get(state, N-1)) return;
