...
def case1(screen, windows, n):
    ...
    total_width = 0
    for i in range(n):
        if screen.H % windows[i].H != 0: return False
...
def case2(screen, windows, n):
    ...
    """
    if n <= 1: return False
    if screen.W % windows[0].W != 0: return False
    win0_height = (screen.W // windows[0].W) * windows[0].H
    if win0_height > screen.H: return False
...
def check(screen, windows, n):
    if n == 0: return True
    if check_helper(screen, windows, n): return True
...
while True:
    ...
    test_no += 1
    if ans: print("Set {}: Yes".format(test_no))
    else: print("Set {}: No".format(test_no))
