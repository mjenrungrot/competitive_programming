...
int conv(tuple<char,char> x){ return conv(get<0>(x), get<1>(x)); }
    ...
    for(int i=0;i<N;i++){
        ...
        for(int j=3;j<line.length();j++){
            if(line[j] != '='){
                ...
                V[conv(curr)].push_back({conv(next), 1});
                V[conv(next)].push_back({conv(curr), 1});

                cout << curr << " to " << next << endl;
                curr = next;
            }else{
                ...
                for(int k1=0;k1<stations.size();k1++) for(int k2=k1+1;k2<stations.size();k2++){
                    V[conv(stations[k1])].push_back({conv(stations[k2]), 2});
                    V[conv(stations[k2])].push_back({conv(stations[k1]), 2});
                }
            }
        }
    }
