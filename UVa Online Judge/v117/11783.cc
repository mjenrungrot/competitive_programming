...
#  FileName:        11783.cc
#  Description:     UVa Online Judge - 11783
=============================================================================*/
#include <bits/stdc++.h>
...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        line2d L1 = points_to_line(A[i].first, A[i].second);

        for (int j = i + 1; j < N; j++) {
            line2d L2 = points_to_line(A[j].first, A[j].second);

            point2d int_point;
            if (are_intersect(L1, L2, int_point)) {
                if (int_point.x - min(A[i].first.x, A[i].second.x) >= -1e-6 and
                    max(A[i].first.x, A[i].second.x) - int_point.x >= -1e-6 and
                    int_point.y - min(A[i].first.y, A[i].second.y) >= -1e-6 and
                    max(A[i].first.y, A[i].second.y) - int_point.y >= -1e-6 and
                    int_point.x - min(A[j].first.x, A[j].second.x) >= -1e-6 and
                    max(A[j].first.x, A[j].second.x) - int_point.x >= -1e-6 and
                    int_point.y - min(A[j].first.y, A[j].second.y) >= -1e-6 and
                    max(A[j].first.y, A[j].second.y) - int_point.y >= -1e-6) {
                    ans++;
                    is_intersect[i] = is_intersect[j] = true;
                }
            }
        }
    }
...
int main() {
    ...
    while (cin >> N) {
        ...
        for (int i = 1; i <= N; i++) {
            cin >> x1 >> y1 >> x2 >> y2;
            A.emplace_back(point2d(x1, y1), point2d(x2, y2));
        }
        solve();
    }
