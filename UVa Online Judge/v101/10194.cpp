...
#  FileName:        10194.cpp
#  Description:     UVa Online Judge - 10194
=============================================================================*/
#include <algorithm>
#include <cstdio>
#include <cstring>
...
struct team {
    ...
    void clear() {
        point = n_game = win = tie = loss = goal_dif = goal_score =
            goal_against = 0;
        name = "";
    }
    ...
    while (tt--) {
        ...
        for (int i = 1; i <= N; i++)
            printf("%d) %s %dp, %dg (%d-%d-%d), %dgd (%d-%d)\n", i,
                   A[i].name.c_str(), A[i].point, A[i].n_game, A[i].win,
                   A[i].tie, A[i].loss, A[i].goal_dif, A[i].goal_score,
                   A[i].goal_against);
        if (tt) printf("\n");
    }
    return 0;
