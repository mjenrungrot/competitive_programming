...
#  FileName:        10256.cc
#  Description:     UVa Online Judge - 10256
=============================================================================*/
#include <bits/stdc++.h>
...
bool inside(polygon::point2d P1, polygon::point2d P2, polygon::point2d Q1,
            polygon::point2d Q2) {
    bool Q1_on_P12 = polygon::cross(P2 - P1, Q1 - P1) > EPS;
    bool Q2_on_P12 = polygon::cross(P2 - P1, Q2 - P1) > EPS;
    bool P1_on_Q12 = polygon::cross(Q2 - Q1, P1 - Q1) > EPS;
    bool P2_on_Q12 = polygon::cross(Q2 - Q1, P2 - Q1) > EPS;

    if (Q1_on_P12 != Q2_on_P12 and P1_on_Q12 != P2_on_Q12) return true;
    return false;
}

void solve() {
    polygon::polygon2d P_N, P_M;
    for (int i = 1; i <= N; i++) {
        double x, y;
        cin >> x >> y;
        P_N.points.emplace_back(x, y);
    }
    for (int i = 1; i <= M; i++) {
        double x, y;
        cin >> x >> y;
        P_M.points.emplace_back(x, y);
    }

    polygon::polygon2d hull_N = P_N.convex_hull();
    polygon::polygon2d hull_M = P_M.convex_hull();

    int hull_N_size = (int)hull_N.points.size() - 1;
    int hull_M_size = (int)hull_M.points.size() - 1;

    for (int i = 0; i < hull_N_size - 1; ++i)
        for (int j = 0; j < hull_M_size - 1; ++j) {
            bool check = inside(hull_N.points[i], hull_N.points[i + 1],
                                hull_M.points[j], hull_M.points[j + 1]);
            if (check) {
                cout << "No" << endl;
                return;
            }
    ...
    for (auto& p : P_N.points) {
        if (hull_M.inside_polygon(p) >= 0) {
            cout << "No" << endl;
            return;
        }
    }

    for (auto& p : P_M.points) {
        if (hull_N.inside_polygon(p) >= 0) {
            cout << "No" << endl;
            return;
        }
    }
