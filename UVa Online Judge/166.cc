...
int dp(int curr_val, int coin_idx, vector<vector<int>> &memo)
{
    ...
    int ans = INF;
    for (int counter = 0; counter <= coin_counts[coin_idx];
         counter++)
    {
        ans = min(ans, dp(curr_val - counter * coins[coin_idx], coin_idx + 1, memo) + counter);
    }
...
int main()
{
    ...
    while (true)
    {
        ...
        if (all_of(coin_counts.begin(), coin_counts.end(), [](int x) { return x == 0; }))
            break;
