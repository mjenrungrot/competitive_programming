...
#  FileName:        11265.cc
#  Description:     UVa Online Judge - 11265
=============================================================================*/
#include <bits/stdc++.h>
...
void solve() {
    ...
    double W, H, target_x, target_y;

    cin >> W >> H >> target_x >> target_y;
    polygon::point2d target(target_x,target_y);

    poly.points.push_back(polygon::point2d(0.0, 0.0));
    poly.points.push_back(polygon::point2d(W, 0.0));
    poly.points.push_back(polygon::point2d(W, H));
    poly.points.push_back(polygon::point2d(0.0, H));
    poly.points.push_back(polygon::point2d(0.0, 0.0));

    for (int i = 1; i <= N; i++) {
        double x1, y1, x2, y2;
        cin >> x1 >> y1 >> x2 >> y2;
        polygon::point2d P1(x1, y1), P2(x2, y2);

        // target point must be left side of line P1-P2
        if(cross(P2-P1, target-P1) > EPS){
            poly = poly.cut_polygon(P1, P2);
        }else{
            poly = poly.cut_polygon(P2, P1);
        }
    }
