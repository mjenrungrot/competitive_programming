...
#  FileName:        11297.cc
#  Description:     UVa Online Judge - 11297
=============================================================================*/
#include <bits/stdc++.h>
using namespace std;
...
void update_x(int px, int py, int Lx, int Rx, int x, int val, int row,
              bool single_row) {
    if (Lx == Rx) {
        if (single_row)
            min_st[py][px] = max_st[py][px] = val;
        else {
            min_st[py][px] = min(min_st[ll(py)][px], min_st[rr(py)][px]);
            max_st[py][px] = max(max_st[ll(py)][px], max_st[rr(py)][px]);
        }
    ...
    int m = (Lx + Rx) >> 1;
    if (x <= m)
        update_x(ll(px), py, Lx, m, x, val, row, single_row);
    else
        update_x(rr(px), py, m + 1, Rx, x, val, row, single_row);
    min_st[py][px] = min(min_st[py][ll(px)], min_st[py][rr(px)]);
    max_st[py][px] = max(max_st[py][ll(px)], max_st[py][rr(px)]);
}
void update_y(int py, int Ly, int Ry, int y, int x, int val) {
    ...
    int m = (Ly + Ry) >> 1;
    if (y <= m)
        update_y(ll(py), Ly, m, y, x, val);
    else
        update_y(rr(py), m + 1, Ry, y, x, val);
    update_x(1, py, 0, N - 1, x, val, -1, false);
}

void query_x(int px, int py, int Lx, int Rx, int x1, int x2, int &min_val,
             int &max_val) {
...
void query_y(int py, int Ly, int Ry, int y1, int y2, int x1, int x2,
...
void build() { build_y(1, 0, N - 1); }
void update(int y, int x, int val) { update_y(1, 0, N - 1, y, x, val); }
