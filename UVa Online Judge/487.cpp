...
int cmp(string A, string B){
	if((int)A.length() != (int)B.length()) return (int)A.length() < (int)B.length();
	return A < B;
}
...
void f(int r,int c,int len){
	...
	pass[r][c] = true;
	for(int k=0;k<8;k++){
		if(r+dify[k] > N or r+dify[k] < 1 or c+difx[k] > N or c+difx[k] < 1) continue;
		if(pass[r+dify[k]][c+difx[k]]) continue;
		if(T[r+dify[k]][c+difx[k]] > T[r][c]) f(r+dify[k],c+difx[k],len+1);
	}
	pass[r][c] = false;
}
