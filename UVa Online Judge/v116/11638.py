...
def run():
    ...
    for i in range(4):
        ...
        if (1 << (i + 4)) & C:
            isHighTriggered[i] = True
        else:
            isHighTriggered[i] = False
    ...
    for (t, temp) in A:
        ...
        if t >= M:
            ...
            if (left_bound - S) % M == 0:
                pass
            elif (left_bound + t - S) % M < (left_bound - S) % M:
                pass
            else:
                continue
        ...
        for i in range(4):
            if not isOn[i]:
                continue
            if isHighTriggered[i] and temp > thresh[i]:
                ans[i] = "yes"
            if not isHighTriggered[i] and temp < thresh[i]:
                ans[i] = "yes"
