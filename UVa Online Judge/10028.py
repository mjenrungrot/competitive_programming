...
def run():
    ...
    while True:
        ...
        while points < 0 and (yyyy + 1, mm, dd) <= (new_yyyy, new_mm, new_dd):
            ...
            if points < 0: print("{:04d}-{:02d}-{:02d} {} demerit point(s).".format(yyyy, mm, dd, -points))
            elif points == 0:  print("{:04d}-{:02d}-{:02d} No merit or demerit points.".format(yyyy, mm, dd))

        while points >= 0 and points + 1 <= 5 and (yyyy + 2, mm, dd) <= (new_yyyy, new_mm, new_dd):
            yyyy += 2
            points += 1
        ...
        if points > 0 and val > 2 * points:
            val -= 2 * points
            points = -val
            print("{:04d}-{:02d}-{:02d} {} demerit point(s).".format(yyyy, mm, dd, -points))
        elif points > 0 and val <= 2 * points:
            points = math.floor(points - val / 2)
            if points > 0: print("{:04d}-{:02d}-{:02d} {} merit point(s).".format(yyyy, mm, dd, points))
            else: print("{:04d}-{:02d}-{:02d} No merit or demerit points.".format(yyyy, mm, dd))
        else:
            points -= val
            print("{:04d}-{:02d}-{:02d} {} demerit point(s).".format(yyyy, mm, dd, -points))
