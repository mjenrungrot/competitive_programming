...
int main(){
	...
	while(T--){
		...
		do{
			...
			for(int i=0;i<4;i++) if(tmp[i] == str1[i]) tmp1++, mask |= (1<<i);
			for(int i=0;i<4;i++) if(tmp[i] != str1[i]){
				for(int j=0;j<4;j++){
					if(i == j) continue;
			...
			tmp1 = tmp2 = mask = 0;
			for(int i=0;i<4;i++) if(tmp[i] == str2[i]) tmp1++, mask |= (1<<i);
			for(int i=0;i<4;i++) if(tmp[i] != str2[i]){
				for(int j=0;j<4;j++){
					if(i == j) continue;
					if(mask&(1<<j)) continue;
