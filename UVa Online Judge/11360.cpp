	...
	for(int i=1;i<=TT;i++){
		...
		for(int j=1;j<=M;j++){
			...
			if(!strcmp(str,"row")){
				...
				for(int k=1;k<=N;k++) for(int l=1;l<=N;l++) T[k][l] = (T[k][l] < '9') ? T[k][l] + 1 : '0';
			}else if(!strcmp(str,"dec")){
				for(int k=1;k<=N;k++) for(int l=1;l<=N;l++) T[k][l] = (T[k][l] > '0') ? T[k][l] - 1 : '9';
			}else if(!strcmp(str,"transpose")){
				for(int k=1;k<=N;k++) for(int l=k+1;l<=N;l++) swap(T[k][l],T[l][k]);
			}
		}
