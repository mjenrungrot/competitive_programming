...
G(S1 S2) = sum_{(a,b) in S1 S2} (a + 1) * (|S1 S2| - b)
         = sum_{(a,b) in S1} (a + 1) * (|S1 S2| - b) + sum_{(a,b) in S2} (a + 1)
* (|S1 S2| - b) + sum_{a in S1 and b in S2} (a + 1) * (|S1 S2| - b) = sum_{(a,b)
in S1} (a + 1) * (|S1| - b) + sum_{(a,b) in S1} (a + 1) * |S2| + sum_{(a,b) in
S2} (a - |S1| + 1) * (|S1 S2| - b) + sum_{(a,b) in S2} (|S1 S2| - b) * |S1| +
                (right[S1] + 1) * (|S1 S2| - (|S1| + left[S2]))
         = G(S1) + |S2| sum_{(a,b) in S1} (a+1) +
                G(S2) + |S1| sum_{(a,b) in S2} (|S1 S2| - b) +
                (right[S1] + 1) * (|S1 S2| - (|S1| + left[S2]))
         = G(S1) + |S2| (sum_left(S1) + n_pairs(S1)) +
           G(S2) + |S1| (|S1 S2| n_pairs(S2) - (|S1| * n_pairs(S2) +
sum_right(S2))) + (right(S1) + 1) * (|S1 S2| - (|S1| + left(S2)))
    ...
    for (int i = 0; i < commands.length(); i++) {
        if (commands[i] == '.') {
            ...
            if (left_ch[i] == 0) {
                ...
                sum_left[i + 1] = 2 * sum_left[i] + n_pairs[i] * len[i] +
                                  (left_ch[i] == right_ch[i] ? 0LL : R[i]);
                sum_right[i + 1] =
                    2 * sum_right[i] + n_pairs[i] * len[i] +
                    (left_ch[i] == right_ch[i] ? 0LL : L[i] + len[i]);
                n_pairs[i + 1] =
                    n_pairs[i] * 2 + (left_ch[i] == right_ch[i] ? 0LL : 1LL);
                ...
                ans[i + 1] +=
                    len[i] *
                    ((len[i + 1] * n_pairs[i]) % MOD -
                     (len[i] * n_pairs[i] + sum_right[i]) % MOD + MOD) %
                    MOD;
                ans[i + 1] = (ans[i + 1] % MOD + MOD) % MOD;
                ans[i + 1] += (left_ch[i] == right_ch[i]
                                   ? 0LL
                                   : (R[i] + 1) * (len[i + 1] - len[i] - L[i]));
            }
        } else {
            // update length
            len[i + 1] = len[i] + 1;
            ...
            if (left_ch[i] == 0) {
                ...
                if (commands[i] == 'X' or commands[i] == 'O') {
                    ...
                    sum_left[i + 1] =
                        sum_left[i] +
                        ((right_ch[i] == commands[i]) ? 0LL : R[i]);
                    sum_right[i + 1] =
                        sum_right[i] +
                        ((right_ch[i] == commands[i]) ? 0LL : R[i + 1]);
                    n_pairs[i + 1] =
                        n_pairs[i] + ((right_ch[i] == commands[i]) ? 0LL : 1LL);
                    ans[i + 1] =
                        ans[i] + (sum_left[i] + n_pairs[i]) +
                        ((right_ch[i] == commands[i]) ? 0LL : R[i] + 1);
                } else {
                    R[i + 1] = R[i];
                    right_ch[i + 1] = right_ch[i];
        ...
        //     debug(i, L[i+1], R[i+1], len[i+1], n_pairs[i+1], sum_left[i+1],
        //     sum_right[i+1], ans[i+1]);
        // }
    }
