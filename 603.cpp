...
int main(){
	...
	while(T--){
		...
		while(fgets(str,sizeof(str),stdin)){
			...
			bool found = false;
			while(true){
				for(int i=0;i<car.size();i++) if(answer[car[i].second] == -1 and car[i].first == val){
		...
		for(int i=0;i<initial.size();i++){
			if(answer[initial[i]] == -1) printf("Original position %d did not park\n",initial[i]);
			else printf("Original position %d parked in %d\n",initial[i],answer[initial[i]]);
		}
	}
	return 0;
}